...
const functionTakesDestructObjectParam = ({ x, y }: { x: number; y: number }) =>
  console.log(`${x} and ${y}`)
const functionTakesAPointObjectParam = (pt: { x: number; y: number }) =>
  console.log(`${pt.x} and ${pt.y}`)
const printOptional = (obj: { firstName: string; surname?: string }) =>
  console.log(`${obj.firstName}${obj.surname}`)
...
const functionTakesAPointAliasTypeWithOptionalYProperty = (pt: Point) => {
  return pt
}
...
const sorter = (a: Distance, b: Distance): -1 | 0 | 1 =>
  a === b ? 0 : a < b ? -1 : 1
...
// type annotation for object goes after destructuring syntax
